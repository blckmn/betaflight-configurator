 ...
 *
 * Messages:
 *   Input:  { type: 'analyze', data: ArrayBuffer, logIndex?: number }
...
self.onmessage = function (event) {
    const { type, data, logIndex } = event.data;
    ...
    try {
        ...
        const idx = logIndex != null && logIndex < logs.length ? logIndex : 0;
        const { start, end } = logs[idx];

        // 2. Parse the log header + data
        postProgress("parsing", `Parsing log ${idx + 1} of ${logs.length}...`);
        const { sysConfig, chirpData } = parseChirpLog(bytes, start, end);
        ...
        let segmentSize = 1;
        while (segmentSize < halfSecondSamples) segmentSize <<= 1;
